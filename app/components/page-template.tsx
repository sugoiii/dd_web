...
export function PageTemplate({
  ...
  const headerClass = fullWidth
    ? cn(
        "flex flex-col border-b sm:flex-row sm:items-end sm:justify-between",
        headingVariant === "compact" ? "gap-1.5 pb-2" : "gap-2 pb-3",
      )
    : cn(
        "flex flex-col border-b sm:flex-row sm:items-end sm:justify-between",
        headingVariant === "compact" ? "gap-2 pb-3" : "gap-3 pb-4",
      );
  const titleClass = headingVariant === "compact"
    ? "text-xl font-semibold tracking-tight text-foreground sm:text-2xl"
    : "text-2xl font-semibold tracking-tight text-foreground sm:text-3xl";
  const descriptionClass = headingVariant === "compact"
